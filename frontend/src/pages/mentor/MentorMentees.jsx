    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <section className="space-y-6">
        <h2 className="text-lg font-bold flex items-center gap-2">
          Active Mentees
          <span className="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-tighter">Verified</span>
        </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mentees.map((mentee) => (
              <div key={mentee._id} className="mentor-card p-6 flex flex-col items-center text-center">
                <div className="w-20 h-20 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 font-black text-2xl mb-4 shadow-inner">
      ...
      <section className="space-y-6 pt-4">
        <h2 className="text-lg font-bold flex items-center gap-2">
          ...
          {pendingRequests.length > 0 && (
            <span className="bg-amber-100 text-amber-700 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-tighter animate-pulse">{pendingRequests.length} Pending</span>
          )}
        </h2>
        ...
        {pendingRequests.length === 0 ? (
          ...
          <div className="space-y-4">
            {pendingRequests.map((request) => (
              <div key={request._id} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center text-amber-600 font-bold text-lg">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-primary-900 text-white text-xs font-bold rounded-xl hover:bg-primary-800 transition-colors disabled:opacity-50"
                  >
